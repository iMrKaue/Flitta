...
func GetHourPerformanceHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, "método não permitido", http.StatusMethodNotAllowed)
		return
	}
	...
	hours, err := repository.GetHourPerformance(clientID)
	if err != nil {
		http.Error(w, "erro ao buscar indicadores por horário", http.StatusInternalServerError)
		return
	}
